...
<li><span class="link-container"><a class="reference external" href="changelog/index.html">Changes and Migration</a></span></li>
    ...
    <div id="docs-body" class="withsidebar errors" >
...
that objects have what they need up front.</p>
<p>When using this approach, it is usually necessary that the
<code class="xref py py-paramref docutils literal notranslate"><span class="pre">Session.expire_on_commit</span></code> parameter be set to <code class="docutils literal notranslate"><span class="pre">False</span></code>, so
that after a <code class="xref py py-meth docutils literal notranslate"><span class="pre">Session.commit()</span></code> operation, the objects within the
session aren’t <a class="reference internal" href="glossary.html#term-expired"><span class="xref std std-term">expired</span></a>, which would incur a lazy load if their
attributes were subsequently accessed.  Additionally, the
<code class="xref py py-meth docutils literal notranslate"><span class="pre">Session.rollback()</span></code> method unconditionally expires all contents in
the <code class="xref py py-class docutils literal notranslate"><span class="pre">Session</span></code> and should also be avoided in non-error scenarios.</p>
<div class="admonition seealso">
<p class="admonition-title">See also</p>
...
<p><a class="reference internal" href="orm/session_basics.html#session-committing"><span class="std std-ref">Committing</span></a> - background on session commit</p>
<p><a class="reference internal" href="orm/session_state_management.html#session-expire"><span class="std std-ref">Refreshing / Expiring</span></a> - background on attribute expiry</p>
</div>
</li>
</ul>
</div>
...
<div class="section" id="for-relationship-relationship-delete-orphan-cascade-is-normally-configured-only-on-the-one-side-of-a-one-to-many-relationship-and-not-on-the-many-side-of-a-many-to-one-or-many-to-many-relationship">
<span id="error-bbf0"></span><h3>For relationship &lt;relationship&gt;, delete-orphan cascade is normally configured only on the “one” side of a one-to-many relationship, and not on the “many” side of a many-to-one or many-to-many relationship.<a class="headerlink" href="#for-relationship-relationship-delete-orphan-cascade-is-normally-configured-only-on-the-one-side-of-a-one-to-many-relationship-and-not-on-the-many-side-of-a-many-to-one-or-many-to-many-relationship" title="Permalink to this headline">¶</a></h3>
...
<span class="k">class</span> <span class="nc">B</span><span class="p">(</span><span class="n">Base</span><span class="p">):</span>
    ...
    <span class="n">a</span> <span class="o">=</span> <span class="n">relationship</span><span class="p">(</span><span class="s2">&quot;A&quot;</span><span class="p">,</span> <span class="n">back_populates</span><span class="o">=</span><span class="s2">&quot;bs&quot;</span><span class="p">)</span></pre></div>
</div>
...
<p><span class="versionmodified changed">Changed in version 1.3.18: </span>The text of the “delete-orphan” error message
...
<div class="section" id="instance-instance-is-already-associated-with-an-instance-of-instance-via-its-attribute-attribute-and-is-only-allowed-a-single-parent">
<span id="error-bbf1"></span><h3>Instance &lt;instance&gt; is already associated with an instance of &lt;instance&gt; via its &lt;attribute&gt; attribute, and is only allowed a single parent.<a class="headerlink" href="#instance-instance-is-already-associated-with-an-instance-of-instance-via-its-attribute-attribute-and-is-only-allowed-a-single-parent" title="Permalink to this headline">¶</a></h3>
...
to the error message described at <a class="reference internal" href="#error-bbf0"><span class="std std-ref">For relationship &lt;relationship&gt;, delete-orphan cascade is normally configured only on the “one” side of a one-to-many relationship, and not on the “many” side of a many-to-one or many-to-many relationship.</span></a>, and the issue is in
